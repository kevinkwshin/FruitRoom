...
TIME_STEP_MINUTES = 60 # 1시간 단위

# 기본값 설정
DEFAULT_AUTO_ASSIGN_START_TIME = time(11, 0)
DEFAULT_AUTO_ASSIGN_END_TIME = time(13, 0)
DEFAULT_AUTO_ASSIGN_SLOT_STR = f"{DEFAULT_AUTO_ASSIGN_START_TIME.strftime('%H:%M')} - {DEFAULT_AUTO_ASSIGN_END_TIME.strftime('%H:%M')}"
...
WEDNESDAY_AUTO_ASSIGN_SLOT_STR = f"{WEDNESDAY_AUTO_ASSIGN_START_TIME.strftime('%H:%M')} - {WEDNESDAY_AUTO_ASSIGN_END_TIME.strftime('%H:%M')}" # (24:00으로 표시해도 무방) -> "21:00 - 00:00" 로 표시
...
if st.session_state.current_page == "🗓️ 예약 시간표 및 수동 예약":
    ...
    # 선택된 날짜의 요일 확인 (0: 월요일, 1: 화요일, 2: 수요일, ..., 6: 일요일)
    selected_weekday = timetable_date.weekday()
    is_wednesday_selected_timetable = (selected_weekday == 2)

    # 시간표 표시 범위 설정
    timetable_display_start_hour = DEFAULT_AUTO_ASSIGN_START_TIME.hour # 기본 11시
    timetable_display_end_hour = DEFAULT_MANUAL_RESERVATION_END_HOUR # 기본 17시
    if is_wednesday_selected_timetable:
        timetable_display_end_hour = WEDNESDAY_AUTO_ASSIGN_END_TIME.hour + 1 # 수요일은 24시까지 표시 (00시)
        if WEDNESDAY_AUTO_ASSIGN_END_TIME.minute > 0 : # 분 단위까지 고려
    ...
    if not reservations_df.empty:
        ...
        if not day_reservations.empty:
            ...
            time_slots_v8 = []
            current_dt_v8 = datetime.combine(today_kst, time(timetable_display_start_hour, 0))
            # timetable_display_end_hour가 24면 time(0,0) of next day가 되어야 함.
            # 여기서는 time객체만 사용하므로 23시까지 루프 돌고, 23시 슬롯을 표시
            # 만약 timetable_display_end_hour가 24라면 루프 조건은 < time(0,0)이 아니라 해당 시간까지 포함하도록
            # 또는 end_of_day_dt_v8을 time(0,0)으로 만들고 루프는 < 로 처리
            
            # 시간표 끝 시간을 datetime 객체로 만듭니다.
            # 만약 timetable_display_end_hour가 24라면 다음날 0시를 의미
            if timetable_display_end_hour == 24:
                # end_of_day_dt_v8 = datetime.combine(today_kst + timedelta(days=1), time(0,0)) # 다음날 00:00
                # 시간표는 당일 23:00 슬롯까지 표시하면 되므로 아래와 같이 유지
                 end_of_day_dt_v8 = datetime.combine(today_kst, time(23,0)) + timedelta(hours=1) # 23:00 슬롯까지 포함
            else:
                 end_of_day_dt_v8 = datetime.combine(today_kst, time(timetable_display_end_hour, 0))
            ...
            # timetable_display_end_hour가 24이고, time_slots_v8에 23:00이 없다면 추가 (00:00 슬롯은 만들지 않음)
            if timetable_display_end_hour == 24 and (not time_slots_v8 or time_slots_v8[-1] != time(23,0)):
                 if time(23,0) not in time_slots_v8 : time_slots_v8.append(time(23,0))
            ...
            for _, res_v8 in day_reservations.iterrows():
                ...
                for slot_start_time_obj in time_slots_v8:
                    # 슬롯 시간과 예약 시간을 비교하기 위해 같은 날짜로 통일 (여기서는 today_kst 사용)
                    ...
                    # 종료 시간이 00:00인 경우 다음날로 처리해야 하지만, 여기서는 당일 23:59로 간주
                    if res_end_time == time(0,0): # 자정 마감 예약 처리 (데이터가 이렇게 저장된다면)
                        res_end_dt_combined = datetime.combine(today_kst + timedelta(days=1), time(0,0))
                    else:
    ...
    with cols_main_reserve_v8[0]:
        selected_team_main_reserve_v8 = st.selectbox("조 선택", ALL_TEAMS, key="manual_team_sel_main_page_reserve_v8" + str(is_wednesday_manual)) # key에 요일 정보 추가하여 리렌더링 유도
        ...
        # 시작 시간이 current_manual_start_hour 보다 이전이면 조정
        ...
        manual_start_time_main_reserve_v8 = st.time_input(
            ...
            key="manual_start_time_main_page_reserve_v8" + str(is_wednesday_manual) # key 변경
        )

    with cols_main_reserve_v8[1]:
        selected_room_main_reserve_v8 = st.selectbox("방 선택", ALL_ROOMS, key="manual_room_sel_main_page_reserve_v8" + str(is_wednesday_manual)) # key 변경
        ...
        # 최대 종료 시간은 current_manual_end_hour
        ...
        if end_time_default_val_v8 > max_possible_end_time_val_v8: # 최대값 초과 방지
        ...
        manual_end_time_main_reserve_v8 = st.time_input(
            ...
            step=timedelta(hours=1),
            key="manual_end_time_main_page_reserve_v8" + str(is_wednesday_manual) # key 변경
        )

    time_valid_main_reserve_v8 = True
    # 유효성 검사 시 current_manual_start_hour, current_manual_end_hour 사용
    ...
    # max_possible_start_time_val_v8은 이미 current_manual_end_hour를 기준으로 계산됨
    ...
    if st.button("✅ 예약하기", key="manual_reserve_btn_main_page_reserve_v8"  + str(is_wednesday_manual), type="primary", use_container_width=True, disabled=not time_valid_main_reserve_v8): # key 변경
    ...
    if not my_manual_res_display_cancel_v8.empty:
        ...
        for _, row_main_cancel_v8 in my_manual_res_display_cancel_v8.iterrows():
            ...
            with item_cols_main_cancel_v8[1]:
                if st.button("취소", key=f"cancel_{res_id_main_cancel_v8}_main_page_reserve_v8" + str(is_wednesday_manual), use_container_width=True): # key 변경
    ...
    current_auto_assign_slot_str = f"{current_auto_assign_start_time.strftime('%H:%M')} - {time(current_auto_assign_end_time.hour + (1 if current_auto_assign_end_time.minute==59 else 0), 0).strftime('%H:%M') if current_auto_assign_end_time != time(0,0) else '00:00'}"
    if is_wednesday_auto_assign and current_auto_assign_end_time == time(23,59): # 수요일 자정 표현
        current_auto_assign_slot_str = f"{current_auto_assign_start_time.strftime('%H:%M')} - 00:00"


    if current_test_mode_admin:
        st.info("🧪 테스트 모드: 요일 제한 없이 자동 배정 가능합니다.")
    else:
        st.info(f"🗓️ 자동 배정은 수요일 또는 일요일에만 실행 가능합니다. (선택된 날짜: {'수요일' if is_wednesday_auto_assign else '수요일 아님'})")
    ...
    # 자동 배정 가능 조건은 (테스트 모드 활성화) 또는 (수요일 또는 일요일)
    # can_auto_assign_admin_page_v8 = current_test_mode_admin or (weekday_admin_page_v8 in [2, 6]) # 2:수, 6:일
    # 위에서 weekday_admin_page_v8는 auto_assign_date_admin_page_v8 (선택된 날짜) 기준
    ...
    if st.button("✨ 선택 날짜 자동 배정 실행", key="auto_assign_btn_admin_page_final_v8", type="primary", disabled=not can_auto_assign_admin_page_v8):
        ...
        if not existing_auto_admin_page_v8.empty:
            st.warning(f"이미 {auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')} {current_auto_assign_slot_str}에 자동 배정 내역이 있습니다.")
        else:
            new_auto_list_admin_page_v8 = []
            assigned_info_admin_page_v8 = []
            # 시니어조 고정 배정은 항상 현재 자동 배정 시간대로
            if SENIOR_TEAM in ALL_TEAMS and SENIOR_ROOM in ALL_ROOMS:
                new_auto_list_admin_page_v8.append({
                    "날짜": auto_assign_date_admin_page_v8, "시간_시작": current_auto_assign_start_time, "시간_종료": current_auto_assign_end_time,
    ...
    auto_today_display_admin_page_v8 = reservations_df[
        ...
        (reservations_df["시간_종료"] == current_auto_assign_end_time) & # 수요일 자정 처리 유의
    ...
    if not auto_today_display_admin_page_v8.empty:
        ...
        st.info(f"{auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')} {current_auto_assign_slot_str} 시간대 자동 배정 내역이 없습니다.")


elif st.session_state.current_page == "📖 관리자 매뉴얼":
    st.header("📖 관리자 매뉴얼")
    # 매뉴얼 내용도 요일별 설정에 맞춰 업데이트
    # (생략 - 이전과 유사하게 동적 정보 반영 필요시 업데이트)
    st.markdown(f"""
    ...
    1.  **예약 시간표 및 수동 예약 (기본 페이지):**
        *   **시간표 조회:** 접속 시 오늘 날짜(KST)가 기본으로 선택되며, 특정 날짜를 선택하여 해당 날짜의 전체 예약 현황을 **1시간 단위** 시간표 형태로 볼 수 있습니다.
            *   시간표는 선택된 날짜에 따라 표시 범위가 조정될 수 있습니다 (수요일은 23:00 슬롯까지 표시).
        *   **수동 예약 등록:**
            ...
            *   원하는 날짜, 조, 방, 시작/종료 시간을 **1시간 단위**로 선택하여 직접 예약할 수 있습니다.
            *   최소 예약 시간은 1시간입니다.
        *   **수동 예약 취소:** (기존과 동일)
    ...
    2.  **자동 배정 (관리자 전용):**
        ...
        *   **배정 시간:**
            *   **수요일:** **{WEDNESDAY_AUTO_ASSIGN_START_TIME.strftime('%H:%M')} ~ {time(WEDNESDAY_AUTO_ASSIGN_END_TIME.hour + (1 if WEDNESDAY_AUTO_ASSIGN_END_TIME.minute == 59 else 0),0).strftime('%H:%M') if WEDNESDAY_AUTO_ASSIGN_END_TIME != time(0,0) else '00:00'}** (만약 23:59면 00:00으로 표시)
            *   **그 외 요일 (일요일 포함):** **{DEFAULT_AUTO_ASSIGN_SLOT_STR}**
        *   **실행 요일:** 수요일, 일요일 (테스트 모드 시 요일 제한 없음)
        *   (고정/로테이션 배정, 실행 방법 등은 기존과 유사)
