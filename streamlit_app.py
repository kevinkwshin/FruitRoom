...
# --- Google Sheets 클라이언트 및 워크시트 초기화 (st.cache_resource 사용) ---
# 이 함수들은 호출 시점에 Streamlit 명령으로 간주될 수 있으므로, 
# st.set_page_config() 이후에 정의하고, 실제 호출은 필요할 때 합니다.
@st.cache_resource
def init_gspread_client():
    try:
        ...
        # st.error는 Streamlit 명령어이므로, 이 함수가 모듈 로딩 시 바로 실행되면 안 됨.
        # 앱 실행 로직 내에서 호출될 때 오류를 표시하도록 변경 가능.
        # print(f"Google Sheets 클라이언트 초기화 실패: {e}") # 로그로 변경 또는 앱 로직에서 처리
        raise RuntimeError(f"Google Sheets 클라이언트 초기화 실패: {e}") # 예외 발생시켜 앱 로직에서 처리
...
def get_worksheets(_gc_client):
    ...
    try:
        ...
        # print(f"Google Sheets 워크시트 가져오기 실패: {e}")
        raise RuntimeError(f"Google Sheets 워크시트 가져오기 실패: {e}")


# --- 데이터 로드 및 저장 함수 (st.cache_data 사용) ---
@st.cache_data(ttl=180)
def get_all_records_as_df_cached(_ws, expected_headers, _cache_key_prefix, gsheet_available_flag):
    if not gsheet_available_flag or _ws is None: return pd.DataFrame(columns=expected_headers)
    try:
        ...
        # st.warning도 Streamlit 명령어.
        # print(f"'{_ws.title}' 시트 로드 중 오류 (캐시 사용 시도): {e}")
        # 오류 발생 시 빈 데이터프레임 반환은 유지
        return pd.DataFrame(columns=expected_headers)


def update_worksheet_from_df(_ws, df, headers, gsheet_available_flag):
    if not gsheet_available_flag or _ws is None: return
    try:
        df_to_save = df.copy()
        ...
        # 캐시 클리어는 해당 함수 호출 시점에 이루어짐
        if _ws.title == "reservations": get_all_records_as_df_cached.clear()
        elif _ws.title == "rotation_state": load_rotation_state_cached.clear() # load_rotation_state_cached로 변경
    except Exception as e:
        st.error(f"'{_ws.title}' 시트 업데이트 중 오류: {e}") # 이 st.error는 버튼 클릭 등 사용자 인터랙션 후 호출되므로 괜찮음

# load_reservations, load_rotation_state 등은 GSHEET_AVAILABLE 상태를 확인 후 호출해야 함.
def load_reservations(gsheet_available_flag, reservations_worksheet):
    if not gsheet_available_flag: return pd.DataFrame(columns=RESERVATION_SHEET_HEADERS)
    return get_all_records_as_df_cached(reservations_worksheet, RESERVATION_SHEET_HEADERS, "reservations_cache", gsheet_available_flag)

@st.cache_data(ttl=180) # load_rotation_state_cached 함수명 유지
def load_rotation_state_cached(_rotation_ws, _cache_key_prefix, gsheet_available_flag): # gsheet_available_flag 추가
    if not gsheet_available_flag or _rotation_ws is None: return 0
    df_state = get_all_records_as_df_cached(_rotation_ws, ROTATION_SHEET_HEADER, _cache_key_prefix, gsheet_available_flag)
    if not df_state.empty and "next_team_index" in df_state.columns:
        try: return int(df_state.iloc[0]["next_team_index"])
        except (ValueError, TypeError): return 0
    return 0

def load_rotation_state(gsheet_available_flag, rotation_worksheet):
    if not gsheet_available_flag: return 0
    return load_rotation_state_cached(rotation_worksheet, "rotation_state_cache", gsheet_available_flag)

def save_reservations(df, gsheet_available_flag, reservations_worksheet):
    if not gsheet_available_flag: return
    update_worksheet_from_df(reservations_worksheet, df, RESERVATION_SHEET_HEADERS, gsheet_available_flag)

def save_rotation_state(next_team_index, gsheet_available_flag, rotation_worksheet):
    if not gsheet_available_flag or rotation_worksheet is None: return
    df_state = pd.DataFrame({"next_team_index": [next_team_index]})
    update_worksheet_from_df(rotation_worksheet, df_state, ROTATION_SHEET_HEADER, gsheet_available_flag)
...
# --- 앱 실행 로직 시작 ---
# Google Sheets 초기화 시도 및 상태 확인
GSHEET_AVAILABLE = False
gc_client = None
reservations_ws = None
rotation_ws = None
...
try:
    ...
    st.error(str(e)) # 초기화 실패 시 에러 메시지 표시
    # GSHEET_AVAILABLE은 False로 유지됨

# 세션 상태 초기화
if "current_page" not in st.session_state:
    st.session_state.current_page = "🗓️ 예약 시간표 및 수동 예약"
if "auto_date_admin_page_final_v8_value" not in st.session_state: # 자동 배정 날짜 세션 상태
    st.session_state.auto_date_admin_page_final_v8_value = get_today_kst()
...
if st.sidebar.button("🔄 데이터 캐시 새로고침", key="cache_refresh_btn_admin_v8"):
    # 캐시 클리어 함수들이 GSHEET_AVAILABLE 상태에 의존하지 않도록 직접 호출
...
if not GSHEET_AVAILABLE:
    # 이 st.error는 GSHEET_AVAILABLE이 False일 때만 실행되므로, set_page_config 이후에 와도 괜찮음.
    if not gc_client: # init_gspread_client에서 예외가 발생한 경우
         pass # 이미 st.error가 try-except 블록에서 표시되었을 수 있음 (또는 여기서 다시 표시)
    elif not reservations_ws or not rotation_ws: # get_worksheets에서 예외가 발생한 경우
         pass
    else: # 일반적인 연결 실패 (예: secrets 설정 오류로 gc_client는 생성됐으나 워크시트 접근 불가)
        st.error("Google Sheets에 연결할 수 없거나 워크시트를 찾을 수 없습니다. 설정을 확인하고 페이지를 새로고침해주세요.")
    st.stop() # GSHEET 연결 안되면 앱 중단

reservations_df = load_reservations(GSHEET_AVAILABLE, reservations_ws)
today_kst = get_today_kst() # 이 함수는 Streamlit 명령을 호출하지 않으므로 위치 괜찮음
...
if st.session_state.current_page == "🗓️ 예약 시간표 및 수동 예약":
    ...
    def style_timetable(df_in):
        styled_df = df_in.style.set_properties(**{
            ...
            'min-width': '85px', 'height': '60px', 'font-size': '0.9em', 'line-height': '1.5'
        }).set_table_styles([
            {'selector': 'th', 'props': [
        ...
        if not day_reservations.empty:
            for _, res_v8 in day_reservations.iterrows():
                ...
                for slot_start_time_obj in time_slots_v8:
                    ...
                    elif res_end_time == time(23,59) and is_wednesday_selected_timetable:
    ...
    manual_date_main_reserve_v8 = st.date_input("예약 날짜", value=manual_date_default_v8, min_value=today_kst, key="manual_date_main_page_reserve_v8")
    ...
    with cols_main_reserve_v8[0]:
        ...
        manual_start_time_main_reserve_v8 = st.time_input("시작 시간", value=start_time_default_val_v8, step=timedelta(hours=1), key="manual_start_time_main_page_reserve_v8" + key_suffix)
    with cols_main_reserve_v8[1]:
        ...
        manual_end_time_main_reserve_v8 = st.time_input("종료 시간", value=end_time_default_val_v8, step=timedelta(hours=1), key="manual_end_time_main_page_reserve_v8" + key_suffix)
    ...
    if st.button("✅ 예약하기", key="manual_reserve_btn_main_page_reserve_v8"  + key_suffix, type="primary", use_container_width=True, disabled=not time_valid_main_reserve_v8):
        current_reservations_main_reserve_v8 = load_reservations(GSHEET_AVAILABLE, reservations_ws)
        ...
        if not is_overlap_main_reserve_v8:
            ...
            save_reservations(updated_df_main_reserve_v8, GSHEET_AVAILABLE, reservations_ws)
        ...
        for _, row_main_cancel_v8 in my_manual_res_display_cancel_v8.iterrows():
            ...
            with item_cols_main_cancel_v8[1]:
                if st.button("취소", key=f"cancel_{res_id_main_cancel_v8}_main_page_reserve_v8" + key_suffix, use_container_width=True):
                    current_on_cancel_main_reserve_v8 = load_reservations(GSHEET_AVAILABLE, reservations_ws)
                    ...
                    save_reservations(updated_on_cancel_main_reserve_v8, GSHEET_AVAILABLE, reservations_ws)
    ...
    # 메시지 표시를 위한 날짜 가져오기 (세션 또는 오늘 날짜)
    # on_change 콜백에서 auto_date_admin_page_final_v8_value를 업데이트 하므로, 이 값을 사용
    auto_assign_date_for_msg = st.session_state.get("auto_date_admin_page_final_v8_value", today_kst)
    weekday_for_msg = auto_assign_date_for_msg.weekday()
    is_wednesday_for_msg = (weekday_for_msg == 2)

    if current_test_mode_admin: 
        st.info("🧪 테스트 모드: 요일 제한 없이 자동 배정 가능합니다.")
    else: 
        day_name_for_msg = "수요일" if is_wednesday_for_msg else ("일요일" if weekday_for_msg == 6 else "평일")
        st.info(f"🗓️ 자동 배정은 수요일 또는 일요일에만 실행 가능합니다. (선택된 날짜: {auto_assign_date_for_msg.strftime('%Y-%m-%d')}, {day_name_for_msg})")

    def update_auto_assign_date_session(): # 날짜 변경 시 세션 상태 업데이트 콜백
        st.session_state.auto_date_admin_page_final_v8_value = st.session_state.auto_date_admin_page_final_v8_widget
        
    auto_assign_date_admin_page_v8 = st.date_input(
        "자동 배정 실행할 날짜", 
        value=st.session_state.auto_date_admin_page_final_v8_value, # 세션 값 사용
        key="auto_date_admin_page_final_v8_widget", # 위젯 키
        on_change=update_auto_assign_date_session 
    )
    ...
    with st.expander("ℹ️ 자동 배정 안내 (클릭하여 보기)", expanded=False):
        day_name_auto_assign = "수요일" if is_wednesday_auto_assign else ("일요일" if weekday_admin_page_v8 == 6 else "평일")
        st.markdown(f"""
        - **선택된 날짜 ({auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')}, {day_name_auto_assign})**
    ...
    if st.button("✨ 선택 날짜 자동 배정 실행", key="auto_assign_btn_admin_page_final_v8", type="primary", disabled=not can_auto_assign_admin_page_v8):
        current_reservations_admin_page_v8 = load_reservations(GSHEET_AVAILABLE, reservations_ws)
        ...
        if not existing_auto_admin_page_v8.empty: st.warning(f"이미 {auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')} {current_auto_assign_slot_str}에 자동 배정 내역이 있습니다.")
            ...
            next_idx_admin_page_v8 = load_rotation_state(GSHEET_AVAILABLE, rotation_ws)
            ...
            for i in range(available_slots_for_rotation):
                if num_rotation_teams_admin_page_v8 == 0: break # 추가: 로테이션 팀 없으면 중단
            ...
            if new_auto_list_admin_page_v8:
                ...
                save_reservations(updated_df_admin_page_v8, GSHEET_AVAILABLE, reservations_ws)
                ...
                save_rotation_state(new_next_idx_admin_page_v8, GSHEET_AVAILABLE, rotation_ws)
    ...
    auto_reservations_for_day = reservations_df[
    ...
    if not auto_reservations_for_day.empty: 
        st.dataframe(auto_reservations_for_day[["조", "방", "예약ID"]].sort_values(by="방"), use_container_width=True)
        st.markdown("---")
        st.subheader("🛠️ 자동 배정 수정 (전체 취소)")
        st.warning(f"아래 버튼은 **{auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')} {current_auto_assign_slot_str}** 시간대의 모든 자동 배정 내역을 삭제합니다. 신중히 사용하세요.")
        if st.button(f"🗑️ {auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')} 자동 배정 전체 취소", key="cancel_all_auto_assign_btn", type="secondary"):
            if not auto_reservations_for_day.empty:
                current_reservations_df = load_reservations(GSHEET_AVAILABLE, reservations_ws)
                ids_to_cancel = auto_reservations_for_day["예약ID"].tolist()
                updated_reservations_df = current_reservations_df[~current_reservations_df["예약ID"].isin(ids_to_cancel)]
                save_reservations(updated_reservations_df, GSHEET_AVAILABLE, reservations_ws)
                st.success(f"{auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')} {current_auto_assign_slot_str} 자동 배정 내역이 모두 취소되었습니다. 필요시 '수동 예약' 페이지에서 다시 등록하세요.")
                st.rerun()
            else: st.info("취소할 자동 배정 내역이 없습니다.")
    ...
    2.  **자동 배정 (관리자 전용):**
        *   **안내 메시지:** 페이지 상단에 자동 배정 가능 요일 및 현재 선택된 날짜의 요일 정보가 표시됩니다.
        *   **배정 시간:**
            *   **수요일:** **{wed_slot_str_manual}**
            *   **그 외 요일 (일요일 포함):** **{default_slot_str_manual}**
        *   **자동 배정 수정 (취소):**
            *   자동 배정 현황 표 아래에 "선택 날짜의 자동 배정 전체 취소" 버튼이 제공됩니다.
    (이하 생략)
    """)
