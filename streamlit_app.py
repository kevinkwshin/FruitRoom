...
if st.session_state.current_page == "🗓️ 예약 시간표 및 수동 예약":
    ...
    # 시간표 생성 로직 (day_reservations 유무와 관계없이 실행)
    ...
    # 수정1: 안내 메시지 위치 변경을 위해 날짜 선택 전에 메시지 표시 로직 구성
    temp_auto_assign_date_for_msg = st.session_state.get("auto_date_admin_page_final_v8_value", today_kst) # 임시로 현재 날짜 또는 이전 선택값 사용
    temp_weekday_for_msg = temp_auto_assign_date_for_msg.weekday()
    temp_is_wednesday_for_msg = (temp_weekday_for_msg == 2)

    if current_test_mode_admin: 
        st.info("🧪 테스트 모드: 요일 제한 없이 자동 배정 가능합니다.")
    else: 
        st.info(f"🗓️ 자동 배정은 수요일 또는 일요일에만 실행 가능합니다. (현재 선택된/기본 날짜: {temp_auto_assign_date_for_msg.strftime('%Y-%m-%d')}, {'수요일' if temp_is_wednesday_for_msg else '수요일 아님'})")

    auto_assign_date_admin_page_v8 = st.date_input(
        "자동 배정 실행할 날짜", 
        value=st.session_state.get("auto_date_admin_page_final_v8_value", today_kst), # 세션 상태 값 유지 또는 오늘 날짜
        key="auto_date_admin_page_final_v8",
        on_change=lambda: st.session_state.update(auto_date_admin_page_final_v8_value=st.session_state.auto_date_admin_page_final_v8) # 값 변경시 세션 업데이트
    )
    ...
    else: end_str_auto = current_auto_assign_end_time.strftime('%H:%M')
    ...
    auto_reservations_for_day = reservations_df[
        ...
        (reservations_df["시간_시작"] == current_auto_assign_start_time) & # 현재 설정된 자동 배정 시간
        (reservations_df["시간_종료"] == current_auto_assign_end_time) &
        (reservations_df["예약유형"] == "자동")
    ]
    if not auto_reservations_for_day.empty: 
        st.dataframe(auto_reservations_for_day[["조", "방", "예약ID"]].sort_values(by="방"), use_container_width=True) # 예약 ID도 표시

        # 수정2: 선택 날짜의 자동 배정 전체 취소 버튼 추가
        st.markdown("---")
        ...
        if st.button(f"🗑️ {auto_assign_date_admin_page_v8.strftime('%Y-%m-%d')} 자동 배정 전체 취소", key="cancel_all_auto_assign_btn", type="secondary"):
            if not auto_reservations_for_day.empty:
                current_reservations_df = load_reservations()
                # 해당 날짜, 해당 자동 배정 시간대의 "자동" 예약을 제외한 나머지 예약들만 남김
                # (주의: 다른 시간대의 자동 예약은 유지됨)
                ...
                save_reservations(updated_reservations_df)
    ...
    2.  **자동 배정 (관리자 전용):**
        ...
        *   **실행 요일:** 수요일, 일요일 (테스트 모드 시 요일 제한 없음)
        *   **자동 배정 실행:** "선택 날짜 자동 배정 실행" 버튼으로 배정.
        *   **자동 배정 수정 (취소):**
            *   자동 배정 현황 표 아래에 "선택 날짜의 자동 배정 전체 취소" 버튼이 제공됩니다.
            *   이 버튼을 누르면 해당 날짜, 해당 자동 배정 시간대의 모든 자동 배정 내역이 삭제됩니다.
            *   삭제 후, 필요한 경우 "예약 시간표 및 수동 예약" 페이지에서 수동으로 다시 예약할 수 있습니다.
